...
# 读取MLPClassifier配置文件
try:
    config = toml.load("./config/experiment/ccRCC_imm_MLP.toml")
...
na.setup_logging(log_file=log_file, experiment_name="mlp")
...
# 保存nadata对象到配置的输出目录
output_file = os.path.join(output_dir, "ccRCC_imm_MLP.pkl")
nadata.save(output_file, format=config['training']['save_format'], save_data=config['training']['save_data'])
logger.info(f"✅ 已完成mlp模型训练，并保存到: {output_file}")
